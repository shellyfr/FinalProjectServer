...
class DBManager:
    ...
    def updateCodes(self, id, time1, x):
        ...
        if x=='3':
            script.update_db("UPDATE codes SET start_argu ='" + time1 + "'  WHERE id= " + id + ";")
        else:
            script.update_db("UPDATE codes SET end_argu ='" + time1 + "'  WHERE id= " + id + ";")
    ...
    def insertDemo1(self, id, age1, gender1,education1, income1,computer1, phone1 ):
        script.insertDB("INSERT INTO demografic1(entry_code, age, gender,education, income,computer, phone) "
                        "VALUES('%s', '%s' , '%s','%s', '%s', '%s', '%s')" %
